...
async function renderWithCallback(req: Request, res: Response) {
  ...
  const {variables, callbackUrl, settings, projectFile} = req.body;
  ...
  try {
    await renderVideo({
      projectFile: projectFile || process.env.PROJECT_FILE || '',
      variables,
      settings: {
        ...settings,
        workers: settings?.workers || parseInt(process.env.WORKERS || '8'),
...
async function renderWithoutCallback(req: Request, res: Response) {
  // TODO: validate request body
  const {variables, streamProgress, settings, projectFile} = req.body;
