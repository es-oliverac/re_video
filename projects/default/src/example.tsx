...
/**
 * Some example data to use in scene when no data is provided
 */
const exampleData = [
...
export default makeScene2D('scene 1', function* (view) {
  ...
  // Resize rectangle to reveal scene
  yield* rectRef().width(0, videoLength);

  // Make rectangle transparent and cover scene again
  rectRef().fill('#00000000');
  rectRef().width(view.width() * 2);

  // Cover scene while Layout block
  // is centered
  yield* all(
  ...
  // Add text with total number of stars
